...
<head>
    ...
    <title>Register - My Support Tracker</title>
...
.ocean-btn:focus-visible .label2 {
  ...
  text-shadow: 0 0 10px var(--border-hover), 0 0 20px var(--glow-secondary);
        ...
        .field.has-toggle input { right: 88px; }
        ...
        .clear {
            scale: 0;
            opacity: 0;
            transition: scale .3s cubic-bezier(.34,1.56,.64,1), opacity .2s ease;
        }
        .field.filled .clear { scale: 1; opacity: 1; }
        .field.has-toggle.filled .clear { right: 48px; }
        .clear:hover, .toggle:hover { background: #232633; }
        .clear svg, .toggle svg { width: 16px; height: 16px; }
        ...
        .field.active .toggle path, .field.active .toggle circle { stroke: var(--accent-4); }
        .toggle .slash {
            stroke-dasharray: 26;
...
<body>
    ...
    <div class="register-card">
        ...
        <div class="card-form">
            <div>
                ...
                <span class="welcome-pill">Get Started</span>
                ...
                <form method="POST" action="{{ route('register') }}">
                    ...
                    <!-- 1. FULL NAME -->
                    <div class="field" data-label="Full Name" data-type="name">
                        <div class="box">
                            <input type="text" 
                                   name="name" 
                                   value="{{ old('name') }}" 
                        ...
                        <div class="helper">
                            <span class="msg">e.g. John Doe</span>
                            @error('name')
                    ...
                    <!-- 4. CONFIRM PASSWORD -->
                    <div class="field has-toggle" data-label="Confirm Password" data-type="confirm-password">
                        <div class="box">
                            <input type="password" 
                        ...
                        <div class="helper">
                            <span class="msg">Re-enter password</span>
                    ...
                    <div class="form-actions">
                        <a href="{{ route('login') }}" class="form-link">Already registered? Log in</a>
                    </div>
                </form>
            </div>
        </div>
    ...
    const cursor_wlc = document.querySelector('.cursor');
        const text_wlc = "Join the My Application Support team — Create an account to start logging daily support tasks, tracking shift handovers, and generating activity reports. ";
        let i_wlc = 0;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            fields.forEach((field) => {
                ...
                function validate() {
                    ...
                    if (field.dataset.type === 'name' && filled) {
                        invalid = val.trim().length < 2;
                        if (msg) msg.textContent = filled ? (invalid ? 'Enter full name' : 'Valid name') : 'e.g. John Doe';
                    }
                    ...
                    if (field.dataset.type === 'password' && filled) {
                        ...
                        if (msg) msg.textContent = filled ? (invalid ? 'Minimum 8 characters' : 'Password set') : 'At least 8 characters';
                    }
                    if (field.dataset.type === 'confirm-password' && filled) {
                        const passVal = document.getElementById('password')?.value || '';
                        invalid = val !== passVal;
                        if (msg) msg.textContent = filled ? (invalid ? 'Passwords do not match' : 'Passwords match') : 'Re-enter password';
                    }
                ...
                input.addEventListener('input', () => {
                    validate();
                    // Re-check confirm password if main password changes
                    if (field.dataset.type === 'password') {
                        const confirmField = document.querySelector('[data-type="confirm-password"]');
                        if (confirmField) {
                            const event = new Event('input');
                            confirmField.querySelector('input').dispatchEvent(event);
                        }
                    }
                });
